...
public class Helpers {
  ...
  private Helpers() {
  } // impedisce di istanziare questa classe, che ha solo metodi statici di utilità
  ...
  private static final Pattern ELEMENT_NUM_PATTERN = Pattern
      .compile("\\s*(?<symbol>\\p{Upper}\\p{Lower}*)\\s*(?<num>\\d+)?\\s*");
  private static final Pattern SC_FORMULA_PATTERN = Pattern
      .compile("\\s*(?<sc>\\d+)?\\s*(?<formula>\\p{Upper}(\\p{Lower}|\\p{Upper}|\\d|\\s)*)\\+?");
   ...
   *
   * <p>
   * Ad esempio, sse invocata sulla stringa <samp>"H2O"</samp> restituisce
   * l'array <samp>["H", "2", "O", "1"]</samp> (si noti la stringa
   * <samp>"1"</samp>
   ...
   *
   * <p>
   * Ad esempio, sse invocata sulla stringa <samp>"2 H2O + NaCl + 3O"</samp>
      ...
      public boolean hasNext() {
        ...
        if (m.start() != lastEnd)
          throw new IllegalArgumentException(
              "La stringa contiene una parte che non corrisponde al formato cercato a partire da \""
                  + source.substring(lastEnd) + "\"");
        lastEnd = m.end();
        last = m;
        return true;
      }
